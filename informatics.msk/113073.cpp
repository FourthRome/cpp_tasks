...
struct Order
{
    ...
    Order(int overrun, int start, int finish, si &overruns) : overrun(overrun), start(start), finish(finish), overruns(overruns) {}
};
...
struct Comp
{
    ...
    Comp(vector<Order> &v) : vec(v) {}

    bool
    operator()(const int &a, const int&b)
    {
        if (vec[a].overrun == vec[b].overrun) {
            if (vec[a].start == vec[b].start) {
                return vec[a].finish > vec[b].finish;
            } else {
                return vec[a].start < vec[b].start;
            }
        } else {
            return vec[a].overrun > vec[b].overrun;
        }
    }
};
...
int main()
{
    ...
    forn(i, n) {
        ...
        si overruns;
        forn(j, i) {
            if (orders[j].finish > a && orders[j].start < b) {
                ++overrun;
                ++orders[j].overrun;

                overruns.insert(j);
                orders[j].overruns.insert(i);
            }
        }
        orders.push_back(Order(overrun, a, b, overruns));
        left.push_back(i);
    }

    sort(ALL(left), Comp(orders));


    int result = n;
    
    while (orders[left[0]].overrun > 0) {
        int dismiss = orders[left[0]].overrun;
        forn(i, result) {
            if (orders[left[i]].overrun == dismiss) {
                --result;
                forit(it, orders[left[i]].overruns) {
                    --orders[*it].overrun;
                    orders[*it].overruns.erase(left[i]);
                }
                
                orders[left[i]].overrun = 0;
                orders[left[i]].overruns.clear();
            }
        }
